...
name=food_name info=food_name type=string
...
def get_food_list():
    ...
    print(food_name)
    # 开启数据库中的游标功能，得到一个游标对象
    cursor = db.cursor()
    if ' ' or ',' or '，' in food_name:
        name1 = re.split(r'[,，\s^]', food_name)[0]
        name2 = re.split(r'[,，\s^]', food_name)[-1]
        cursor.execute(
            "SELECT * FROM `foodinfo` WHERE `food_name` LIKE '%{}%' OR `food_name` LIKE '%{}%'".format(name1, name2))
    else:
        cursor.execute(
            "SELECT id,food_name from foodinfo WHERE `food_name` LIKE '%{}%'".format(food_name))
    # 查询返回的全部结果
    result = cursor.fetchall()
    if len(result) == 0:
        return control.fail("您的输入有误或搜索的食物未录入，请重新输入！")
    else:
        for i in result:
            dictionary = {
                'id': i[0],
...
"""
function：营养详细信息, methods=["get"]
interface=/api/food/nutrition_details
name=food_nutrition info=食物营养名称 type=string
"""
...
def get_nutrition_details():
    ...
    cursor = db.cursor()
    cursor.execute(
        f"SELECT food_name,{food_nutrition} from foodinfo WHERE '{food_nutrition}' = '{food_nutrition}' ORDER BY {food_nutrition} DESC limit 10")
    result = cursor.fetchall()
    print(result)
    food_list = []
    for i in result:
...
"""
function：获取食物详细信息, methods=["get"]
interface=/api/food/food_detailed
name=food_id info=食物id type=int
"""
...
def get_main_nutrition():
    ...
    cursor = db.cursor()
    cursor.execute(sql)
    name = ['蛋白质', '脂肪', '碳水化合物', '粗纤维', '单不饱和脂肪酸', '多不饱和脂肪酸', '胡萝卜素', '叶黄素类', '钙', '镁', '钠', '钾', '磷','硫', '氯', '铁', '锌', '硒', '铜', '锰', '维生素A', '维生素C', '维生素E', '维生素B1','维生素B2', '维生素B3', '维生素B4','维生素B5', '维生素B6', '维生素B9', '维生素B12', '维生素B14', '亮氨酸', '蛋氨酸', '苏氨酸', '赖氨酸', '色氨酸', '缬氨酸', '组氨酸', '异亮氨酸']
    result = cursor.fetchall()
    ...
    base_nutrition = food[['energy(kcal)', '蛋白质（毫克）', '脂肪（毫克）', '碳水化合物（毫克）', '粗纤维（毫克）']]
